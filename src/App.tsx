  ...
  return (
    <div className="min-h-screen pb-24">
      ...
      <main className="max-w-3xl mx-auto px-4">
        ...
        <div
          className={`fixed bottom-8 left-1/2 -translate-x-1/2 z-50 transition-all duration-300 transform w-[min(90vw,400px)] ${toast.visible ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0 pointer-events-none'
            }`}
        >
          <div className="sketch-panel px-4 md:px-6 py-2 md:py-3 flex items-center gap-3">
            <CheckCircle2 size={20} className="text-brand-accent shrink-0" strokeWidth={3} />
            <span className="font-bold text-base md:text-lg text-ink leading-tight">{toast.message}</span>
          </div>
        </div>
